...
# マスタデータ
# -----------------------------
play_prices = {"60":20000,"90":25000,"120":30000,"150":45000,"180":55000,"210":65000,"240":75000,"270":85000,"300":95000,"330":105000,"オールナイト":120000,"特殊料金":0}
option_prices = {"無し":0,"乳首舐め":2000,"聖水":3000,"ボンデージ":1000,"その他の衣装":1000,"局部奉仕":8000,"アナル奉仕":5000,"その他(特別料金)":0}
location_prices = {"新宿（歌舞伎町）":0,"渋谷（道玄坂）":0,"鶯谷":0,"池袋":1000,"五反田":1000,"錦糸町":1000,"アルファイン":3000,"その他（特別料金）":0}
...
with st.form(key="info_form"):
    ...
    with col1:
        inp_name = st.text_input("名前","")
        inp_email = st.text_input("メールアドレス（任意）","")
        inp_phone = st.text_input("電話番号（任意）","")
    ...
    with col2:
        ...
        inp_other_text = st.text_input("その他（任意）","")
...
def make_reservation_info():
    ...
    lines = ["‐‐‐‐‐‐‐‐","【ご予約内容】",f"{dt.month}月{dt.day}日（{weekday}） {dt.strftime('%H:%M')}〜（{inp_play_time}分枠）",f"場所：{loc_choice}"]
...
choice = st.selectbox("出力するテンプレを選択してください",
                      options=["基本情報","予約情報","DM①（最初）","DM②（カウンセリング後）","DM③（前日確認）","メール①（最初）","メール②（カウンセリング後）","メール③（前日確認）"])

if st.button("生成"):
    if choice=="基本情報": out_text=make_basic_info()
    elif choice=="予約情報": out_text=make_reservation_info()
    elif choice=="DM①（最初）": out_text=make_dm1()
    elif choice=="DM②（カウンセリング後）": out_text=make_dm2()
    elif choice=="DM③（前日確認）": out_text=make_dm3()
    elif choice=="メール①（最初）": out_text=make_mail1()
    elif choice=="メール②（カウンセリング後）": out_text=make_mail2()
    else: out_text=make_mail3()
